...
**Status**: Draft
**Owner**: Braden
**Builds on**: `specs/20260616T225034-reactions-buildout.tracker.md` (V0 done-when), `docs/adr/0014-an-always-on-reaction-runs-app-semantics-beside-the-app-blind-anchor.md`, `docs/adr/0015-agent-conversations-are-durable-child-docs-answered-by-reactions.md`
**Lands on**: a fresh branch off `origin/main` after PR #2077 merges (do not stack on the `codex/...` branch)

## One Sentence

Give the Zhongwen app a daemon entry so its always-on reaction actually runs, then prove the V0 exit in the real app: a daemon answers a conversation bound to its agent, the browser abstains, exactly one answer, cancel survives a disconnect.

## Why Now

PR #2077 lands the always-on reaction primitives and a runnable proof (`examples/doc-as-wire-chat`), but that example proves the model in isolation: a standalone reaction process with no competing transport. The real app, Zhongwen, has never run the daemon, because there is no `apps/zhongwen/epicenter.config.ts` for `epicenter up` to load. So the V0 done-when ("a phone and a desktop see the same streamed reply over hosted sync, cancel works after a disconnect, 0 duplicate streams") is unproven where it matters: in a shipping app, over hosted sync, against the browser's competing HTTP generation path.
...
- **reaction**: the running process that observes docs, thinks, and writes answers.
- **agent**: the durable address a conversation binds to; a reaction answers *as* an agent.

`epicenter.config.ts` is none of these. It is the declaration that names which app a folder runs and which agent its reaction answers as. The daemon loads it; the reaction (the observe loop) responds, and only to conversations whose `row.agent` matches that agent. The doc's `agent` field decides who answers, never the topology.
...
- **Daemon half** (`packages/workspace/src/document/child-doc-reactions.ts`): the observe loop filters `.filter(isDesignated)` where `isDesignated(rowId) = row.agent === selfAgentId`. The daemon hosts only conversations bound to its agent.
...
- Confirm what `packages/server/src/ai/reaction-over-room-sync.test.ts` test 3 currently asserts (one map vs two), so it is known whether any R work remains or it is purely config plus proof.
...
Everything else (nodeId from `.epicenter/node.json`, the Unix socket, the reaction loop, sync) wires automatically. The mount factory already accepts `agentId`, the reaction is already registered (`reactions.conversations.messages`), and the designation predicate is built in `workspace.ts`.
...
Exit: a real two-surface run (or an integration test against a live room, in the shape of `reaction-over-room-sync.test.ts`) showing one answer, durable cancel, and survival across reconnect.

### Slice 3: Collapse pass on the new surface (rides along)

The new files are greenfield. Per the collapse-pass Implementation Gate, audit every new helper, wrapper, and file-split. First targets: the example's `smoke.ts` / `smoke-cancel.ts` / `smoke-binding.ts` trio and the `transport.ts` / `conversations.ts` helpers; plus one-caller boundaries in the new workspace files. Leave the `chat-reaction.ts` and `doc-generation.ts` flush-policy duplication alone: that is the deliberate C4 deferral.
...
The cloudless Iroh anchor (thread 3, `specs/20260616T185740-cloudless-home-anchor-direction.md`) is not part of this. This thread proves the reaction half over the existing hosted relay/anchor; the self-hosted anchor is a later, separate slice that depends on this being solid.
