...
// This is the type that gets stored in Y.Doc via onFinish/sendMessage and
// rendered by MessageParts.svelte. Deriving from UIMessage keeps the drift
...
// ── Typed boundary: persisted JSON → MessagePart[] ─────────────────────

/**
 * Convert a persisted chat message to a TanStack AI UIMessage.
 *
 * This is the single boundary where persisted JSON parts are cast to
 * `MessagePart[]`. Safe because parts are always produced by TanStack AI
 * and round-tripped through Y.Doc serialization (structuredClone-compatible,
 * lossless for plain objects).
 */
