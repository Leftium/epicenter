/**
 * The workspace's substrate-only column builder: what `defineTable` authoring adds on
 ...
 * for the kinds and `nullable` for the substrate policy, with no `column` namespace. A
 * branded format like an IANA timezone is just `field.string<IanaTimeZone>()`: the brand
 * carries the type, no bespoke builder required. The `FlatJsonTSchema` constraint (in
 * `./constraint`) still gates every `defineTable` column, whether authored via `field.*`,
 * `nullable`, or raw `Type.*`.
 */
