...
> - **Production wiring**: `apps/honeycrisp/honeycrisp.browser.ts` (inline composition with per-row child docs), `apps/honeycrisp/src/lib/session.ts` (session glue), `apps/tab-manager/src/lib/session.svelte.ts` (browser extension auth binding)
