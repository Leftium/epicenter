...
**Verdict (2026-06-29): delete iroh, relay-only is the destination.** The product owner settled that offline-LAN is not a requirement, and latency is marginal for request/response tool calls, so neither justification holds. A Codex deletion-ledger pass (plus this session's read) found the cost of keeping iroh is large and standing, not just the native dep: iroh anchors the ENTIRE signed device-key trust ledger (`account/{assertion,crypto,reducer-trust,sas}.ts`, `account-doc`'s verify/revoke/identity, the CLI `devices`/`verify`/`revoke`/`sas` verbs) plus the gateway iroh layer, the route-table device-key thresholds, and the selecting seam, used for nothing else; the relay floor authenticates by the relay-stamped `userId` and a relay-exposed route, touching none of it. The agreed sequence is **build / prove / remove**: the non-destructive smoke knobs (`--via`, `--relay-expose`) land first (this session), the relay floor is proven on two real machines, and only then is iroh deleted. The one real migration is the daemon `nodeId`: today it IS the iroh `EndpointId` and feeds the Yjs `clientID` derivation, so the deletion wave swaps in a non-iroh durable id while preserving the stable `clientID` (decoupling it earlier would break the selecting transport's single `target`). "Which devices exist" moves from the signed roster to server-owned relay presence; offline device labels, if wanted, become non-authoritative presence metadata, never a revived trust ledger.
...
**3. Per-channel auth: the relay enforces routing integrity, the device acceptor is the endpoint gate (honest asymmetry).** The relay refuses cross-owner routing **by construction**, using the already-authenticated `Connection.userId` (lifted from the bearer at upgrade, never client-supplied): it routes only among sockets of the same per-user fleet room. The device endpoint is the real boundary, and the two transports authenticate on **different axes**: iroh authenticates a *device* by its Ed25519 key (Ring 0), the relay authenticates a *user* (it vouches, within ADR-0004's existing relay trust). (Settled by a 2026-06-29 Codex design pass; built in build-step 3.)

The honest threat read: a **cloud-minted bearer does NOT defend against a malicious relay**, because the bearer signer (cloud auth) shares the relay's trust domain; a compromised relay can forge the open, read, inject, and replay bytes. So **do not build a relay-scoped bearer and pretend it is the boundary.** The relay floor's security ceiling is a *trusted relay for routing and byte integrity*; the answer when that is unacceptable is to self-host the relay (ADR-0068), not a wire trick. What the floor *does* enforce, and it is enough for its ceiling:

- the relay stamps a **server-authored `source: { kind: 'user', userId }`** onto the forwarded open, OVERWRITING any caller-provided one, from the authenticated `Connection.userId`. A keyless caller cannot forge it.
- the **daemon acceptor is the endpoint gate** (the stdio `local-books mcp` child has no header slot), the relay equivalent of Ring 0: `createRelayRouteOpener` admits a channel only when `source` is this daemon's own owner AND the route is explicitly `relay: 'exposed'`.
- the route table gains a **`relay: 'exposed' | 'refused'`** policy (default `refused`), SEPARATE from iroh's device-key `requires`, because a keyless browser user can never meet a device-key threshold. A **sensitive route (`books`) stays iroh-only**; a low-risk route is opted in knowingly, accepting the trusted-relay ceiling.

`Origin` applies only to the HTTP-shaped boundaries it exists at (the browser's WS upgrade, a future self-host tunnel); inventing an Origin field inside the raw byte channel would be security theater. A cryptographic endpoint bearer is deferred to the **HTTP-tunnel case**, where a URL is genuinely reachable and must authenticate every request itself.
