...
- [x] **1.2** Add and meter `POST /v1/audio/transcriptions` in `apps/api/worker`, mirroring `chargeOpenAiCreditsWithAutumn`; back it with a cheap cloud STT using the house key. Publish exactly what the gateway does with the bytes (memory-only, no R2, no training, named upstream, zero retention). _Done: library route `packages/server/src/routes/transcription.ts` (OpenAI `whisper-1` via the chat gateway's existing `OPENAI_API_KEY`, forces `verbose_json` for the `duration` the meter reads; pinned off `gpt-4o-transcribe`, which drops `verbose_json`); hosted metering `chargeOpenAiTranscriptionCredits` settles per audio-minute after the call (pre-gate denies an empty wallet, no reservation lock). Chat middleware prefix narrowed `/v1/*` -> `/v1/chat/*` so STT does not inherit chat metering. Self-host Bun host mounts it unmetered._
- [x] **1.3** Wire vocab to resolve the hosted Connection on web and call `transcribe`. Dictation ships on web, zero setup. _Built on `vocab-web-dictation`: a minimal in-app `createRecorder` (MediaRecorder -> Blob) plus a `dictation` singleton that calls `transcribe(blob, resolveOrHosted('whisper-1'), { model: 'whisper-1', language: 'en' })`; the mic is an optional `accessory` snippet on the shared chat input, and the transcript lands in the draft for review. No `#platform/*` seam (vocab is web-only; the seam is only earned if vocab wraps to Tauri). Model + language are app-local constants in `vocab.ts`. Headless green: whole-monorepo typecheck 0 errors, vocab build + tests pass. The only remaining gate is the live mic + signed-in hosted session + `OPENAI_API_KEY`-provisioned smoke (the gateway 503s `ProviderNotConfigured` without the key); when it passes, delete this spec and the handoff._
