...
### 4. `message` is a call-site input, `.withMessage()` is an optional default

This is the biggest change from the intermediate design. `message` is always a valid factory input:

- **Without `.withMessage()`**: `message` is **required** at the call site
- **With `.withMessage()`**: `message` is **optional** — the template provides a default, call site can override

```typescript
// No .withMessage() — message required
...
// With .withMessage() — message optional, has default
...
// With fields + .withMessage() — message computed from fields, overridable
