...
Multi-tenancy (per-user partitions, OAuth, billing) is Epicenter Cloud's only (`apps/api`); an instance never grows a mode, an allowlist, OAuth, sessions, first-boot minting, Better Auth, or a database. The relational-auth substrate (Better Auth + Postgres) is Cloud-only (ADR-0076): the instance composes neither, so it provisions nothing but the token. Named per-person tokens are a deliberately-unbuilt seam (a hashed registry behind the same verifier + the same constant partition); build it only on real offboarding pain, never speculatively.
