 ...
 * derives the provider from the catalog), and the always-on daemon worker builds
 ...
 * always-on worker that answers over sync, so the browser stays out of the way
 ...
 * route). The home daemon is the always-on worker a user co-deploys; binding a
...
const conversationsTable = defineTable({
	 ...
	 * always-on worker over sync, and the browser skips its kickoff. One immutable
