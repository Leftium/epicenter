<script lang="ts">
	...
	$effect(() => {
		if (!paletteOpen) app.state.paletteSearch.reset();
	});
	...
	let wasOpen = false;
	$effect(() => {
		const isOpen = app.state.terminal.open;
		if (wasOpen && !isOpen) {
			previousFocus?.focus();
			previousFocus = null;
		}
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if ((e.metaKey || e.ctrlKey) && e.key === '`') {
			e.preventDefault();
			if (!app.state.terminal.open) {
				previousFocus = document.activeElement as HTMLElement | null;
				app.state.terminal.toggle();
...
<div class="flex h-screen flex-col">
	<Resizable.PaneGroup direction="horizontal" class="flex-1">
		...
		<Resizable.Pane defaultSize={chatOpen ? 45: 75}>
			<Resizable.PaneGroup direction="vertical">
				<Resizable.Pane
					defaultSize={app.state.terminal.open ? 55: 100}
					minSize={30}
				>
					<ContentPanel />
				</Resizable.Pane>
	...
	<CommandPalette
		items={app.state.paletteSearch.searchResults}
		bind:open={paletteOpen}
		bind:value={app.state.paletteSearch.searchQuery}
		placeholder={app.state.paletteSearch.scope === 'names' ? 'Search file names...': app.state.paletteSearch.scope === 'content' ? 'Search content...': 'Search files...'}
		emptyMessage={app.state.paletteSearch.scope === 'content' ? 'No content matches.': app.state.paletteSearch.scope === 'both' ? 'No results.': 'No files found.'}
		title="Search Files"
		description="Search for files by name or content"
		shouldFilter={app.state.paletteSearch.shouldFilter}
	>
				...
				<Tooltip.Root>
					<Tooltip.Trigger>
						{#snippet child({ props })}
							<Toggle
								size="sm"
								pressed={app.state.paletteSearch.scope === 'names'}
								onPressedChange={(v) => { app.state.paletteSearch.scope = v ? 'names': 'both'; }}
				...
				<Tooltip.Root>
					<Tooltip.Trigger>
						{#snippet child({ props })}
							<Toggle
								size="sm"
								pressed={app.state.paletteSearch.scope === 'content'}
								onPressedChange={(v) => { app.state.paletteSearch.scope = v ? 'content': 'both'; }}
