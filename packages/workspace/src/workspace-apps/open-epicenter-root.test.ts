 ...
 * - local-only mounts open without loading Epicenter auth
 * - a local mount that returns collaboration at runtime is rejected
...
describe('openEpicenterRoot', () => {
	...
	test('imports the config and opens every declared mount', async () => {
		writeConfig(
			`export default [
				{ name: 'alpha', kind: 'local', open: () => (${RUNTIME}) },
				{ name: 'beta', kind: 'local', open: () => (${RUNTIME}) },
			];\n`,
		);
	...
	test('rejects a local mount that returns collaboration at runtime', async () => {
		...
		expect(
			await Bun.file(join(epicenterRoot, 'invalid.disposed')).exists(),
		).toBe(true);
	});
	...
	test('requires auth for collaborative mounts before claiming the folder', async () => {
		writeConfig(
		...
		// Auth is decided before the folder claim writes any markers.
		expect(await Bun.file(join(epicenterRoot, '.epicenter')).exists()).toBe(
			false,
		);
	});
	...
	test('refuses a mixed signed-out root before opening local siblings', async () => {
		writeConfig(
			`import { writeFileSync } from 'node:fs';
			import { join } from 'node:path';
			export default [
	...
	test('adopts a populated mount folder once `.epicenter/` exists', async () => {
		...
		writeConfig(
			`export default [{ name: 'fuji', kind: 'local', open: () => (${RUNTIME}) }];\n`,
		);
