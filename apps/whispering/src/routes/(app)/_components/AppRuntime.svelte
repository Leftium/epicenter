<script lang="ts">
	...
	import { commandCallbacks, dispatchCommandTrigger } from '$lib/commands';
	...
	// desktop global backend is separate and started below. Both backends feed
	// the same `dispatchCommandTrigger` sink, which owns the `on` edge filter.
	$effect(() => {
	...
	// (Accessibility), so none of it walls. This is a pure synchronous read that
	// stores nothing, so a returning user is only ever sent back here by deleting
	// their last runtime, where landing on the picker is correct rather than
	// hostile.
	$effect(() => {
	...
	onMount(() => {
		...
		// Push the configured bindings to this platform's one shortcut backend
		// (system-global rdev on desktop, in-app keydown on the browser) and
		// self-heal duplicate bindings. The `#platform/shortcuts` seam resolves to
		// a single backend per platform, so there is no `tauri` branch here.
		void shortcuts.sync();
		shortcuts.resetIfDuplicates();

		// Seed the OS-permission probe and re-check on window focus, then supervise
		// the rdev listener's liveness: start it once Accessibility allows, restart
		// it on an unexpected death, and surface a standing notice when it cannot.
		// Both are no-ops off their platforms (the web build, and non-macOS for the
		// probe), so neither needs a tauri/os guard here.
		cleanupPermissions = permissions.attach();
		cleanupGlobalListener = globalListener.attach();

		// Desktop only: the rdev trigger stream plus the other native listeners.
		// The browser drives shortcuts through the in-app keydown manager (the
		// `$effect` above) and has none of this.
		if (tauri) {
			// Subscribe the command layer to the rdev trigger stream. The thread's
			// liveness (start on grant, restart on death) is owned by
			// `globalListener`, attached above; this stays pure trigger transport.
