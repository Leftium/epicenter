...
- **`define*`** is pure: no Y.Doc, no side effects. Schemas, KV definitions, action factories, or type-preserving wrappers such as `defineWorkspaceBundle`.
...
- `apps/whispering/src/lib/whispering/tauri.ts`: IndexedDB + BroadcastChannel + recording markdown export
