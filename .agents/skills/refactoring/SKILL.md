...
6. **Convenience re-exports**: Implementation files (`create-*.ts`, `define-*.ts`) that re-export types from `types.ts` "for convenience" at the bottom of the file. These accumulate silently—every file seems harmless—but nobody actually imports types via the implementation file. They also leave behind orphaned imports that only existed to feed the re-export. Scan non-barrel files for `export type { ... }` or `export { ... }` lines past the midpoint of the file.
...
- **Convenience re-exports from implementation files**: Adding `export type { Foo, Bar } from './types.js'` at the bottom of `create-foo.ts` so consumers "don't have to know about types.ts." They already do—the barrel `index.ts` handles public API, and `types.ts` is the canonical source for sibling files. The re-export creates a second import path nobody uses, plus orphaned imports at the top of the file that only existed to feed the re-export. This applies to any language with modules—Python's `from .types import Foo` at the bottom of an implementation module, Rust's `pub use` in non-`mod.rs` files. If a file's job is to create something, it should only export what it creates.
