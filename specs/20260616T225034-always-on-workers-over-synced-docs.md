...
1. **The role split** (ADR-0014): an app-blind **anchor**/**relay** for custody and
   ...
   per-app workers.
2. **Doc-as-wire driven by an observing worker** (ADR-0015): the agent turn is a
...
worker (app-aware, ADR-0014/0015), beside the anchor
...
- Chat as the worker's only interface: synthesizing a human-addressed turn (a fake
  ...
  are one durable interface to a worker, not its only one (ADR-0014).
