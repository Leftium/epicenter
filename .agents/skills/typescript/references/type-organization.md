...
- `DocumentHandle` / `DocumentFactory` — previously in their own `define-document.types.ts` file that existed only to keep them out of a bucket. Used only by `define-document.ts`. **Inlined** back into `define-document.ts` — zero reader benefit to the separation, and the separate file's own docstring admitted it was a workaround.
