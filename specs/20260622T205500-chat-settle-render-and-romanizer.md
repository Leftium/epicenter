...
| ~~Keep `{@html}` + DOMPurify for now~~ LANDED: component tree | 3 taste | Replaced with a shared `@epicenter/ui/markdown` renderer | The "clean later move" landed: `marked.lexer` -> a recursive `<MarkdownNode>` tree emits real DOM (native `<ruby>` for romanized leaves), so `{@html}`, `DOMPurify`, and the regex-HTML `annotate()` are all deleted. Link/image URL schemes are scheme-checked in place of DOMPurify's URL filter. Adopted by vocab (pinyin), tab-manager, and opensidian from one component |
...
2. **Where do `Segment`/`Romanizer` live?** RESOLVED: (b), with the shared renderer.
   - Options: (a) `@epicenter/vocab` package; (b) a small shared `@epicenter/...` module if other apps will romanize; (c) local to vocab for now.
   - **Resolution**: (b). The component-tree renderer (Design Decisions, above) is shared in `@epicenter/ui/markdown`, and the renderer is the contract's consumer: it takes `romanizer?: Romanizer` and turns each `Segment` into `<ruby>`. So the contract types live with the renderer, and vocab's `pinyinRomanizer` (still vocab-local, the only romanizer impl) imports `Romanizer`/`Segment` from `@epicenter/ui/markdown`. A vocab-local type would force a duplicated, duck-typed copy across the package boundary, since apps depend on `@epicenter/ui` and not the reverse. This supersedes the earlier (c) recommendation: the shared-renderer decision pulled the contract out of vocab before `@epicenter/vocab` is extracted, because the renderer (not a future vocab package) is the second consumer.
