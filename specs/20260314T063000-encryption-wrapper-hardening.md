...
// set() — no key = none-mode passthrough
const keyBytes = key;
...
1. **Sign-out writes none-mode over ciphertext.** When a user signs out, `key` is `undefined`. New writes go none-mode. A none-mode write with a newer LWW timestamp permanently replaces previously encrypted data—security downgrade via timestamp.
...
type EncryptionMode = 'none' | 'locked' | 'active';
...
|----------|--------|-----------|
| Mode state machine | `none` → `active` ↔ `locked` | `none` is the initial state for workspaces that have never seen a key. Once a key arrives, mode becomes `active`. Key cleared → `locked`. `locked` rejects writes to prevent none-mode overwriting ciphertext. |
| Locked mode behavior | `set()` throws, `get()` returns cached plaintext | Reads should still work (map was populated while active). Writes must fail to prevent security downgrade. |
...
| Mode persistence | Not persisted—derived from key presence | Mode is runtime state. On fresh page load, workspace starts in `none` if no key cache, or `active` if key cache provides a key. No need to store mode. |
...
│    ├─ mode === 'none' → inner.set(key, val)           │
│    └─ mode === 'active' → encrypt + inner.set            │
│                                                             │
...
│    ├─ key present  → mode = 'active', rebuild map        │
│    ├─ key cleared  → mode = 'locked' (if was active)     │
│    └─ key cleared  → mode = 'none' (if was none) │
│                                                             │
...
Note: `none` → `locked` never happens. `locked` means "was active before." A workspace that never had a key stays `none` through sign-out because there's no ciphertext to protect.
...
- [x] **2.1** Add `mode` state (`none` | `locked` | `active`) to `createEncryptedKvLww`. Initialize based on whether `key` is provided at creation time.
- [x] **2.2** Gate `set()` on mode — throw in `locked`, encrypt in `active`, passthrough in `none`.
...
- [x] **3.1** Mode transitions: none → active → locked → active round-trip
...
- [x] **3.4** Key transition: create wrapper without key, add entries as none-mode, call `unlock(key)`, verify new writes encrypt and map is rebuilt
...
2. `unlock(cachedKey)` called immediately → mode = `active`
...
- [x] `set()` throws in `locked` mode, encrypts in `active`, passes through in `none`
...
- **Simplified mode transition**: Replaced the redundant `else if (mode === 'none') { mode = 'none' }` no-op with `else if (mode !== 'none') { mode = 'locked' }`.
