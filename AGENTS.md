...
Post-change review and cleanup: After making code changes, re-read every touched file before final response. For substantial implementations, public API changes, refactors, multi-file changes, or any request to grill or simplify the result, load `post-implementation-review`; if the review finds indirection that should collapse, use `collapse-pass` for the cleanup ritual. Mentally inline changed call sites, helpers, components, props, and lifecycle boundaries. Either collapse unearned boundaries or state the invariant that makes them worth keeping. Flag findings before applying follow-up fixes. A requested lane is not a no-fix rule: fix grounded correctness, invariant, public API, verification, and serious clarity issues you uncover. Leave speculative, cosmetic, or product-direction expansions for follow-up unless the user has invited broader cleanup. Explicit user limits still win.
