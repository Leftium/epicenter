 ...
 * to `never` and the field cannot carry a reaction. `attachChatTranscript` does.
 */
	...
	function mount(mountOptions: MountOptions<TTables, TKv, TActions>): Mount {
		...
		return runtime.defineSessionMount({
			...
			open(ctx) {
				...
				// so a reaction cannot interpret a body with a layout that disagrees
 ...
 *
 * For every `(table, field)` the mount registered a reaction on, read the field's
 ...
 * guid) and shape (the layout) stay single-owner: a reaction can never read a body
