 ...
 *                      check and deduct credits, and queues a refund
 *                      when the handler responds 4xx/5xx. BYOK callers
 *                      bypass billing entirely.
 *   autumnStorageGate  Around `/api/.../assets`. Pre-flights POST
 *                      uploads against storage balance, tracks usage on
 *                      201 responses, releases bytes on 204 DELETE
...
export const autumnAiGate = createMiddleware<Env>(async (c, next) => {
	...
	const outcome = await billing.guardAiChat({
	...
	if (c.res.status >= 400) {
		c.var.afterResponse.push(billing.refundAiCharge(outcome.credits));
	}
});
...
export const autumnStorageGate = createMiddleware<Env>(async (c, next) => {
	...
	if (method === 'POST') {
		...
		const outcome = await billing.guardAssetUpload(file.size);
		if (outcome.kind === 'deny') {
			return c.json(outcome.body, outcome.status);
		}
