...
- **Stale-auth fast-fail**: with an expired token, `epicenter up --dir peer-a` exits as soon as the relay rejects the WebSocket upgrade (one round-trip, no wallclock timer) with `"auth failed: <code>. Try \`epicenter auth login\`."`, never an indefinite hang. The workspace sync layer rejects `whenConnected` with a `SyncFailedError` on close code 4401 (see `20260427T120000-workspace-sync-failed-phase.md`).
