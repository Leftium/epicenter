...
> **Note**: `@epicenter/sync-core` and `@epicenter/sync` implement this protocol. See `packages/sync-core/src/protocol.ts` for the server-side encoding and `packages/sync/src/provider.ts` for the client-side implementation. The article below describes y-sweet's original implementation, which inspired ours.
