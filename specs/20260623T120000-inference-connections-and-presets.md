...
| Presets pre-fill, key never moves | 1 evidence | preset = `{ id, label, baseUrl, requiresKey }` (local-vs-cloud derived from the URL, not stored); key is always `Authorization: Bearer` | verified: every OpenAI-compatible provider takes the key as Bearer; only base URL / key-required / model-list differ. Self-host is a Custom URL, not a preset. |
...
- Collapse the picker's connection plumbing into one shared `createInferenceConnections` registry (in `@epicenter/app-shell`, beside the picker) instead of passing the connection set, the discovered-model cache, and their setters as separate props. The picker drops from 9 props to 4 (`model`, `onSelectModel`, `connections`, `disabled`); the two per-app persisted stores, `buildVocabCandidates`, and the `resolveForModel`+`resolveConnection` pairing all fold into the registry's `add`/`remove`/`cacheModels`/`discover`/`resolve` methods; the arktype schema is the one SSOT. Each app instantiates the factory once with its `storageKey` + hosted catalog + transport, so Phase 3.3 is a factory call, not a store rewrite. The dead `hosted` picker prop and the redundant stored `location` were removed in the same pass (local-vs-cloud is derived from the base URL).
  Revisit when: a third consumer needs a registry capability the five methods do not cover.
