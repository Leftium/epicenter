...
Reference Cloudflare Worker for self-hosting Epicenter as a team. Composes `@epicenter/server` with `team({ isMember })`.
...
- Do not collapse `TEAM_OWNER_ID` into env config: it is byte-pinned durable data (HKDF label, R2 prefix, DO name prefix, IDB prefix).

## When editing

- Changes to composition primitives (`mount*`, `team()`, `personal()`) live in `packages/server`, not here.
