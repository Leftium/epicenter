 ...
 * joins) and AUTO-MOUNTS every relay-exposed route the user's other online devices
 * advertise in presence (`peer.exposedRoutes`). The floor carries tool routes only
 * (ADR-0078), so every advertised route is an MCP server this auto-mounts as a tool
 * catalog. No picker: the consent boundary is the daemon exposing a route
 * (`--relay-expose books`, default refused); a browser is a pure consumer that
 * reflects whatever its fleet exposes. The mounted catalogs compose beside
 * opensidian's in-process action catalog in `session.ts`.
 *
