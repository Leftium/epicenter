...
Multi-tenancy (per-user partitions, OAuth, billing) is Epicenter Cloud's only (`apps/api`); an instance never grows a mode, an allowlist, OAuth, sessions, first-boot minting, Better Auth, or a database. The relational-auth substrate (Better Auth + Postgres) is Cloud-only (ADR-0075): the instance composes neither, so it provisions nothing but the token. Named per-person tokens are a deliberately-unbuilt seam (a hashed registry behind the same verifier + the same constant partition); build it only on real offboarding pain, never speculatively.
...
- Do not add OAuth, sessions, an allowlist, a launch-time mode selector, or first-boot token minting back: the instance is bearer-only by design (ADR-0074).
- Do not re-add Better Auth, Postgres, a `pg` pool, `DATABASE_URL`, `BETTER_AUTH_SECRET`, or a Hyperdrive binding: the relational-auth substrate is Cloud-only (`mountCloudAuth`), and the instance composes neither (ADR-0075). It uses `requireBearerUser` (never `requireCookieOrBearerUser`). The rooms route records no telemetry on any deployment (the write-only `durableObjectInstance` table and its recorder seam were deleted, ADR-0075), so there is no telemetry knob to set here.
