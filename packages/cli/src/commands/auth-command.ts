...
async function readLine(prompt: string, silent = false): Promise<string> {
  ...
  if (silent) {
    ...
    outputStream = new Writable({ write(_, __, cb) { cb(); } });
  } else {
...
function buildLoginCommand(home: string) {
  return {
    ...
    builder: (yargs: Argv) =>
      yargs.option('remote', {
        type: 'string',
        description: 'Remote server URL (e.g. https://my-epicenter.example.com)',
      }),
    handler: async (argv: { remote?: string }) => {
      ...
      try {
        response = await client.post<SignInResponse>('/api/auth/sign-in/email', { email, password });
      } catch (err) {
...
function buildStatusCommand(home: string) {
  return {
    ...
    handler: async () => {
      ...
      try {
        const session = await client.get<SessionResponse>('/api/auth/get-session');
        ...
        console.log(`Logged in as: ${displayName}${auth.user?.email ? ` (${auth.user.email})` : ''} [stored]`);
        console.log(`Remote:       ${auth.remoteUrl}`);
        console.log(`Expires at:   ${auth.expiresAt ? new Date(auth.expiresAt).toLocaleString() : '(unknown)'}`);
        console.warn('Warning: Could not verify session with remote server. Token may be expired or server unreachable.');
      }
    },
  };
}
