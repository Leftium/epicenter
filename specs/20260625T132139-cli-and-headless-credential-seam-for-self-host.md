...
- **Adjacent collapses noted, left for a separate PR (pre-existing, not worsened here):** `machine-auth.ts` is a fat module (path + IO + login/status/logout + client + `fetchApiSession`); and three `/api/session` readers coexist (`read-api-session.ts` shared, `machine-auth.ts`'s `fetchApiSession`, `status`'s inline read) where `fetchApiSession` could delegate to `readApiSession`.
