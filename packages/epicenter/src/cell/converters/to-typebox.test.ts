...
describe('schemaFieldToTypebox', () => {
	describe('text', () => {
		test('accepts strings', () => {
			const field: SchemaFieldDefinition = { name: 'Title', type: 'text', order: 1 };
	...
	describe('richtext', () => {
		test('accepts strings and null', () => {
			const field: SchemaFieldDefinition = { name: 'Content', type: 'richtext', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('integer', () => {
		test('accepts whole numbers', () => {
			const field: SchemaFieldDefinition = { name: 'Count', type: 'integer', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('real', () => {
		test('accepts any number', () => {
			const field: SchemaFieldDefinition = { name: 'Price', type: 'real', order: 1 };
			const schema = schemaFieldToTypebox(field);
			expect(Value.Check(schema, 0)).toBe(true);
	...
	describe('boolean', () => {
		test('accepts true and false', () => {
			const field: SchemaFieldDefinition = { name: 'Active', type: 'boolean', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('date', () => {
		test('accepts any string (no strict validation)', () => {
			const field: SchemaFieldDefinition = { name: 'Created', type: 'date', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('datetime', () => {
		test('produces string schema (same as date)', () => {
			const field: SchemaFieldDefinition = { name: 'Updated', type: 'datetime', order: 1 };
			const schema = schemaFieldToTypebox(field);
			expect(Value.Check(schema, '2024-01-01T12:00:00Z')).toBe(true);
			expect(Value.Check(schema, null)).toBe(true);
		});
	...
	describe('select', () => {
		...
		test('falls back to string if no options', () => {
			const field: SchemaFieldDefinition = { name: 'Status', type: 'select', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('tags', () => {
		...
		test('falls back to string array if no options', () => {
			const field: SchemaFieldDefinition = { name: 'Tags', type: 'tags', order: 1 };
			const schema = schemaFieldToTypebox(field);
	...
	describe('json', () => {
		test('accepts any value (Type.Unknown)', () => {
			const field: SchemaFieldDefinition = { name: 'Data', type: 'json', order: 1 };
			const schema = schemaFieldToTypebox(field);
...
describe('schemaTableToTypebox', () => {
	...
	test('rejects invalid field values', () => {
		...
		expect(Value.Check(schema, { title: 'Hello', views: 'not a number' })).toBe(false);
	});
