...
AGPL-3.0-or-later: every app (`apps/api`, `apps/dashboard`, `apps/whispering`, `apps/honeycrisp`, `apps/opensidian`, `apps/fuji`, `apps/zhongwen`, `apps/tab-manager`, `apps/skills`, `apps/reddit`, `apps/landing`, `apps/posthog-reverse-proxy`), the sync protocol (`packages/sync`), and our internal packages (`packages/auth`, `packages/auth-svelte`, `packages/skills`, `packages/ai`, `packages/constants`).
