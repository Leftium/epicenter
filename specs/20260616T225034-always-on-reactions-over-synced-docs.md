...
1. **The role split** (ADR-0014): an app-blind **anchor**/**relay** for custody and
   transport, and an app-aware **reaction** for semantics, running beside it. Epicenter
   Cloud is a relay plus optional managed reactions; a user-owned box is an anchor plus
   per-app reactions.
2. **Doc-as-wire driven by an observing reaction** (ADR-0015): the agent turn is a
   durable doc record, not an HTTP request. The reaction observes, streams into a
...
3. **Durable, multi-device approval**: because the reaction and the approver may be on
...
reaction (app-aware, ADR-0014/0015), beside the anchor
...
No server-to-client SSE exists in this picture. The reaction appends each token to the
`Y.Text` and Yjs sync carries it to every replica; the UI is a doc observer. The
only token stream is model-to-reaction, and with local inference that is in-process.
...
sibling capabilities an `onChange` may call directly. The reaction runtime is agnostic
...
These are not a new taxonomy. They are the order in which the reaction + doc-as-wire
layer lights up the already-decided lanes.

```txt
V0  read-only Q&A            Model 1, query tools only. No mutation, no approval,
                             no sandbox. Reaction streams prose into the transcript.
...
V2  authored workflows       Model 2 exactly: arbitrary TypeScript, full trust,
                             ...
                             reaction. New pieces vs the ai-workflows doc: drive it from
                             any device over doc-as-wire, and wrap any harness behind
                             the reaction adapter. Needs OS isolation; deferred + opt-in.
```

V0 and V1 need no new infrastructure beyond the reaction (the daemon body plus an
...
```txt
TOOLS THE LOCAL BOOKS REACTION EXPOSES
...
READ (auto-approve; cannot mutate)
  books_sql_query({ sql })     read-only SQL on the LOCAL mirror, bounded results.
                               ...
                               pipe; the reaction runs it locally and returns bounded
                               rows or prose.
...
appends a turn and (optionally) nudges your desktop; the desktop reaction runs the model
...
PROVE (hosted sync, no Iroh, extend Zhongwen)
  ...
  2. Build the observer: a local reaction reads the conversations table, opens and
     ...
     Hosting is schema-driven, symmetric with the browser opener: `mount({ reactions })`
  ...
  4. Durable cancel: client writes cancelRequestedAt; reaction (observing mid-gen)
  ...
  5. Real provider/local inference behind startStream(); now the hosted AI route is
     unnecessary for Zhongwen-on-reaction.                                            [V0]
...
- **Harness**: pi (`earendil-works/pi`, TypeScript, RPC mode, `tool_call` approval
  ...
  the same reaction adapter. The adapter maps harness events to the transcript and the
...
- Chat as the reaction's only interface: synthesizing a human-addressed turn (a fake
  ...
  are one durable interface to a reaction, not its only one (ADR-0014).
...
1. RESOLVED (ADR-0015, 2026-06-17): binding is data, not a race. The conversation
   ...
   creation, not a per-install node id). Each reaction's observe loop hosts a live
   ...
   by construction. (Build deferred until a second reaction exists; V0 has one agent.)
