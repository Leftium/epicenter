...
- **`ASSETS_BUCKET` (an R2 object binding) left `ServerBindings`,** because a `process.env`-validatable schema holds only strings. The doomed Cloudflare-only assets route reads it via a local `AssetsEnv` cast; it disappears when assets folds into the portable blob store.
