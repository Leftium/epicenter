# 0014. An always-on worker runs app semantics beside the app-blind anchor

- **Status:** Proposed
- **Date:** 2026-06-16

> **Vocabulary (2026-06-18):** a **worker** is the running behavior that observes workspace state and writes results back. An **agent** is the durable address a worker answers as (ADR-0015). A materializer is an addressless worker. [ADR-0017](0017-durable-storage-is-one-per-person-coordination-box.md) adds the **store** as a fifth app-blind role beside the anchor and commits the single-coordination-box topology.
...
- A **worker** is a running process: the role above that holds a live replica,
  observes, and writes.
- An **agent** is the durable, configuration-authored *address* a conversation
  binds to (`AgentId`, [ADR-0015](0015-agent-conversations-are-durable-child-docs-answered-by-workers.md)),
