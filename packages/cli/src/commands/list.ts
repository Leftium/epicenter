 ...
 *
 * The daemon returns mount-qualified snake_case action keys for every opened
 ...
 * daemon's mount-qualified action surface only.
 *
...
export const listCommand = cmd({
	...
	builder: (yargs) =>
		yargs
			.positional('path', {
				type: 'string',
				describe: 'Optional mount-qualified key to narrow the view',
			})
