...
describe('push', () => {
	test('imports markdown files into workspace tables', async () => {
		...
		const result = await workspace.materializer.actions.markdown_push();
	...
	test('overwrites existing rows (set is insert-or-replace)', async () => {
		...
		const first = await workspace.materializer.actions.markdown_push();
		...
		const second = await workspace.materializer.actions.markdown_push();
...
describe('pull', () => {
	test('writes all valid rows to disk', async () => {
		...
		const result = await workspace.materializer.actions.markdown_pull();
...
describe('round-trip', () => {
	test('pull then push on fresh workspace preserves row data', async () => {
		...
		await workspace1.materializer.actions.markdown_pull();
...
// ============================================================================
// dirtyKey Tests (skip-when-unchanged)
// ============================================================================

describe('dirtyKey', () => {
	test('cold start skips toMarkdown for a row whose key matches the on-disk file', async () => {
		const workspace = createWorkspace({
			id: 'dk-unchanged',
			tables: tableDefinitions,
			kv: {},
		});
		workspace.tables.posts.set({ id: 'p1', title: 'Alpha', published: true });
		// A file already on disk with the same title -> the seeded dirty key
		// matches the current row, so the body must not be re-read.
		await writeTestFile(
			'posts/p1.md',
			'---\nid: p1\ntitle: Alpha\npublished: true\n---\n',
		);
	...
	test('cold start re-materializes a row whose key changed while offline', async () => {
		const workspace = createWorkspace({
			id: 'dk-changed',
			tables: tableDefinitions,
			kv: {},
		});
		workspace.tables.posts.set({ id: 'p1', title: 'Beta', published: true });
		// On-disk title is stale (Alpha) vs the current row (Beta): the seeded
		// key differs, so the row must be re-read and rewritten.
		await writeTestFile(
			'posts/p1.md',
			'---\nid: p1\ntitle: Alpha\npublished: true\n---\n',
		);
		...
		expect(calls).toBe(1);
		const onDisk = parseMarkdownFile(await readTestFile('posts/p1.md'));
		expect(onDisk?.frontmatter.title).toBe('Beta');
		workspace[Symbol.dispose]();
	});
});
