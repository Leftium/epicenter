 ...
 * and the relay channel) uses its socket. The session is `null` when machine auth is
 ...
 * `inactive(reason)`.
 *
 * Factories like `honeycrisp()` return a `Mount`. `name` is a display label only: it
