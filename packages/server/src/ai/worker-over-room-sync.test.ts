/**
 * The always-on chat worker, reconciling a transcript over a REAL room.
 *
 * Every other worker test drives `onChange` by hand over an in-memory `Y.Doc`
 * with no sync. This suite is the missing end-to-end proof: a daemon peer runs
 * `attachChatWorker` over a transcript body that is synced through a live
 ...
 *
 * The worker here always answers because the daemon's observe loop only ever
 * builds it for a conversation bound to this daemon's agent (the loop filters by
 * the row's `agent`; see child-doc-worker.test.ts). The D3 single-answerer
 * guarantee is therefore that loop filter plus the browser skipping its HTTP
 * kickoff for daemon-owned conversations, not anything this worker decides, so it
...
/**
 * Wire `attachChatWorker` to a body and fire `onChange` on every transaction.
 *
 * This stands in for the daemon's observe loop, which builds the worker only for a
 * designated conversation; the worker itself has no designation concept, so the
 * harness just attaches it to the body.
 */
function attachDaemon(
	ydoc: Y.Doc,
	startStream: Parameters<typeof attachChatWorker>[0]['startStream'],
) {
	const transcript = attachChatTranscript(ydoc);
	const worker = attachChatWorker({ ydoc, startStream });
	const unobserve = transcript.observe(() => worker.onChange?.());
	return {
		transcript,
		dispose() {
			unobserve();
...
describe('chat worker over real room sync', () => {
	test('the daemon answers a turn written by another peer, and the reply syncs back', async () => {
		...
		// Exactly one answer: the single designated worker did not double-stream.
	...
	// worker straight to a body. This one runs the REAL observe loop
	// (`attachChildDocWorker`) over a table of two conversations bound to two
	...
	// `workerFor = attachChatWorker`). It proves the daemon answers ONLY the
	...
	test('the daemon answers only the conversation bound to its agent, over sync', async () => {
		...
		const worker = attachChildDocWorker({
			...
			workerFor: ({ ydoc }) =>
				attachChatWorker({ ydoc, startStream: streamOf('你', '好', '!') }),
			isDesignated: (rowId) =>
