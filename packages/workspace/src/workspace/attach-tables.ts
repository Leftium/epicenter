...
export function attachTables<T extends TableDefinitions>(
	...
	const entries = Object.entries(definitions).map(([name, def]) => {
		// Share `AttachPrimitive.Table` with `attachTable` so mixing the two
		// on the same Y.Doc for the same name always throws — the conceptual
		// slot is "table named <name>" regardless of which function attached.
