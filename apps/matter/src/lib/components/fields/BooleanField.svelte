<script lang="ts">
	...
	// A checkbox, not a Select: a boolean is exactly true or false, plus the empty
	// state, and a checkbox shows all three without a popover. checked = true, empty
	// box = false, the minus (indeterminate) = empty. Clicking an indeterminate box
	...
	// between true and false; emptying the cell back to its model empty state is the shared
	...
	// already collapsed "absent key OR bare YAML null" into an empty state (the one
	...
	const indeterminate = $derived(hasNoValue(cell));
</script>
